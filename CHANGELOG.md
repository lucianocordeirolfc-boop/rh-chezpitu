2026-09-07 — Escala de Folga (impressão): folha A4 inteira e layout idêntico nas duas empresas
- [CORREÇÃO] O auto-fit media o layout errado: toda a geometria de impressão vivia dentro de @media print, e applyPrintFitScale mede a folha com o container já no DOM mas ainda em mídia screen. A medição enxergava cabeçalho e logo maiores, campos .no-print ainda visíveis e rodapé mais espaçado, devolvendo altura ~8% a 12% maior que a real (811px medidos contra 755px reais, reproduzido em Chrome headless). O fator saía menor que 1 sem necessidade e a folha era encolhida duas vezes — uma pelo CSS de impressão, outra pelo transform: scale()
- [CORREÇÃO] As regras @media screen and (max-width: 1366px/1180px) da pré-visualização (margin-right: -110mm, margin-bottom: -72mm) alcançavam também a folha de impressão e contaminavam a medição em telas estreitas. Passaram a mirar .scale-print-preview-scroll .scale-print-area
- [CORREÇÃO] A redução do auto-fit era uniforme: encolher para caber nos 210mm de altura encolhia junto a LARGURA, que não precisa ceder (são sempre 30/31 dias mais a coluna de nomes). Com 30 funcionários, só 80,4% da largura da folha era usada. Folha, margens laterais e coluna de nomes passaram a ser desenhadas divididas pelo fator — calc(297mm / var(--scale-print-fit)) — e o transform: scale(fator) as devolve ao tamanho impresso pretendido
- [CORREÇÃO] Como a sobra depende de quantas linhas e setores cada empresa tem, Chez Pitu e Pengold recebiam fatores diferentes e saíam com colunas e fontes diferentes. Nos PDFs de referência de Setembro/2026: 256,1mm contra 280,0mm dos 297mm, colunas de dia de ~7,8mm contra ~8,6mm
- [ARQUITETURA] A geometria da folha saiu de @media print para um bloco próprio, sem media query, ancorado em #scalePrintContainer — container criado e removido por printScale, então as regras só valem durante a impressão e a medição enxerga exatamente o que vai para o papel. Em @media print ficaram apenas as regras de página. Fora da impressão o container fica fora da viewport (position: fixed; left: -20000px), com largura definida, para medir sem piscar na tela
- [MELHORIA] Coluna de nomes fixa em 26mm impressos, independente da faixa de densidade (antes 26/23/20mm conforme o número de funcionários). As faixas de densidade seguem ajustando altura de linha e corpo de fonte, que é o que precisa ceder para caber na folha
- [MELHORIA] Margem lateral da folha de 4mm para 2,5mm — a grade passa a ocupar 292mm dos 297mm
- [MELHORIA] applyPrintFitScale virou um ponto fixo de 3 rodadas, já que o fator entra na própria largura da folha e alargar muda o conteúdo medido. Ao final confere se contentH x fator ainda cabe nos 210mm e aperta o fator se não couber — nunca cortar funcionário nem gerar 2ª página
- [RESULTADO] Medido no vetor dos PDFs gerados: largura usada de 256,1mm (Chez Pitu) e 280,0mm (Pengold) para 297,0mm nas duas; coluna de dia de ~7,8mm / ~8,6mm para 8,86mm nas duas; coluna de nome de 23mm (variável) para 26mm fixos. Nome 26mm e dia 8,86mm em todos os quadros testados (8, 17, 20, 30, 40 e 48 funcionários). Nos dois PDFs de 17 funcionários, as 76 bordas verticais da grade caem no mesmo x — só as cores do tema diferem
- [TESTE] verify-print-escala.mjs marcava body.printing-scale e emulava mídia print ANTES de medir, e por isso o bug passava batido. Reescrito para reproduzir o fluxo real de printScale: mede em mídia screen, sem a classe; só depois marca o body e emula print
- [TESTE] verify-print-escala.mjs 55 -> 119 asserções: medição em screen igual à medição em print (o auto-fit não pode depender da mídia), grade ocupando >=97% dos 297mm da folha, e comparação direta Chez Pitu x Pengold com o mesmo quadro (17 e 30 funcionários) exigindo coluna de nome, coluna de dia, corpo de fonte, altura de linha e fator idênticos
- [PRESERVADO] Pré-visualização na tela, cores dos temas por empresa, garantia de 1 única página A4 paisagem e de nenhum funcionário cortado — todas provadas pela suíte
- [PENDÊNCIA] Na vertical a folha segue alinhada ao topo (Pengold com 15 funcionários usa ~176mm dos 210mm). Preencher a sobra exigiria esticar as linhas, o que faria a altura de linha variar entre as empresas — o oposto do layout idêntico pedido. Mantido de propósito
- Arquivos: css/escala-print.css, js/escala.js, scripts/verify-print-escala.mjs
